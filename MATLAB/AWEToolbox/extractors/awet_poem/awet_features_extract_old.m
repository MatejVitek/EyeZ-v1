function [features] = awet_features_extract_old(I, I_annotations)
    ...
    for i = 1:uniqsLen
        ...
        LBP = lbpMatlab(AEMI, BLOCK_SIZE);
        
        fun = @(bl) histcounts(bl.data, BIN_SIZE);
...
function features = lbpMatlab(I, block_size)
    nFiltSize = 8;
    nFiltRadius = block_size;
    filtR=generateRadialFilterLBP(nFiltSize, nFiltRadius);

    features = efficientLBP(I, 'filtR', filtR, 'isRotInv', false, 'isChanWiseRot', false);
end
