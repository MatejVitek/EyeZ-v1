...
% 
% Description:
% This function provides a wrapper to use the STAPLE fusion method for
...
% *************************************************************************
function fused_features = fuse_features_STAPLE(features, roi_small, prob_values, varargin)
%FUSE_FEATURES_STAPLE Fuse the input features using STAPLE
%   Detailed explanation goes here
    global use_progress_bar;
    % Get the settings
	global settings;
	ls = settings.FusionSettings.STAPLE;	% local settings
    ...
    if use_progress_bar
        ProgressBar.update('new', 'Fusing features ...', 'STAPLE Fusion');
        starttime = tic;
    ...
    for u = 1:no_users
        for f = 1:no_fingers
            for i = 1:no_images
                ...
                svprior=prior;
                % Fusion
                [S_est, S_W, ~] = STAPLE(obs, ls.epsilon_S, svprior, ls.init_flag_S, ls.cf1);
                S_est=double(S_est).*fvrn_small;
                S_est=bwareaopen(S_est, ls.areaopen_1);
                S_est=~bwareaopen(~S_est, ls.areaopen_2);
                % Store in cell array
                if ~prob_values
                    fused_features{u, f, i} = S_est;
                else
                    fused_features{u, f, i} = S_W(:,:,1,2); 
                end
