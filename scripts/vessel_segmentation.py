...
if __name__ == '__main__':
	...
	f1_file = os.path.join('/hdd', 'EyeZ', 'Rot', 'Segmentation', 'Results', 'Vessels_Scores.txt')
	...
	legend_size = 20
	color = iter(['orange', 'red', 'blue', 'green', 'black', 'purple'])
	plt.figure('ROC')
	plt.figure('Zoomed ROC')

	with open(f1_file, 'w') as score:
		for model in ('Miura_MC', 'Miura_RLT', 'Miura_RLTGS', 'Miura_MC_norm', 'Miura_RLT_norm', 'Miura_RLTGS_norm'):
			print(model)
			...
			if load_pr and os.path.isfile(pr_file.format(model)):
				...
				dir = os.path.join(pred_dir, model)
				pred = []
				gt = []
