...
def annotations_to_masks(source_dir, target_dir, save_type='json', plot=False, **kwargs):
	...
	Parallel(n_jobs=-1, backend='multiprocessing')(
		delayed(_process_images)(i, source_dir, target_dir, save_type, all_types, plot)
		for i in os.listdir(source_dir)
	)


def _process_images(i, source_dir, target_dir, save_type, all_types, plot):
	...
	for fname in os.listdir(source):
		...
		for channel in PRIMARY_CHANNELS + SECONDARY_CHANNELS:
			for img_ext in IMG_EXTS:
				...
				if plot:
					fig = plt.figure(num=f'{basename}_{channel}')
					fig.add_subplot(121)
...
if __name__ == '__main__':
	...
	annotations_to_masks(source, target, save_type='img', plot=False, logging_level=logging.WARNING, logging_file='')
